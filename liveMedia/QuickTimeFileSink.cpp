...
public:
    ChunkDescriptor(int64_t offsetInFile,
                    unsigned size,
                    unsigned frameSize,
                    unsigned frameDuration,
                    struct timeval presentationTime);

    ChunkDescriptor* extendChunk(int64_t newOffsetInFile,
                                 unsigned newSize,
...
QuickTimeFileSink::QuickTimeFileSink(UsageEnvironment& env,
          ...
          : Medium(env),
            fInputSession(inputSession),
            fFileNameGen(fileNameGen),
            fPresentationStart({0, 0}),
            fPresentationEnd({0, 0}),
            fFileCallback(fileCallback),
            fFileDuration(fileDuration),
            fBufferSize(bufferSize),
            fPacketLossCompensate(packetLossCompensate),
            fSyncStreams(syncStreams),
            fGenerateMP4Format(generateMP4Format),
            fAreCurrentlyBeingPlayed(False),
            fLargestRTPtimestampFrequency(0),
            fNumSubsessions(0),
            fNumSyncedSubsessions(0),
            fHaveCompletedOutputFile(False),
            fMovieWidth(movieWidth),
            fMovieHeight(movieHeight),
            fMovieFPS(movieFPS),
            fMaxTrackDurationM(0) {
    fOutputFilename = fileNameGen();
    fOutFid = OpenOutputFile(env, fOutputFilename.c_str());
    if (fOutFid == NULL)
        return;
    fCurFileStart = time(nullptr);
...
static long toNS(struct timeval tv) {
...
QuickTimeFileSink::~QuickTimeFileSink() {
    ...
    if (fFileCallback) {
        fFileCallback(fOutputFilename, toNS(fPresentationStart), toNS(fPresentationEnd));
    }
}


void QuickTimeFileSink::rotateOutputFile() {
    // XXX beware of copy-paste from ctor/dtor
    ...
    fOutputFilename = fFileNameGen();
    fOutFid = OpenOutputFile(envir(), fOutputFilename.c_str());
    fCurFileStart = time(nullptr);
    fMaxTrackDurationM = 0;
...
static int isH264iFrame(void* data)
{
    ...
    // XXX inconsistent with #define H264_IDR_FRAME 0x65  //bit 8 == 0, bits 7-6 (ref) == 3, bits 5-0 (type) == 5
    if ((fragment_type == 5) ||
...
static double nsToDouble(long ns)
{
    return ns / (double)1000000000ull;
}
...
::afterGettingFrame(void* clientData, unsigned packetDataSize,
    ...
    if (isH264iFrame(ioState->fBuffer->dataStart()) &&
        ...
        ioState->fOurSink.fPresentationStart = presentationTime;
    ioState->fOurSink.fPresentationEnd = presentationTime;

    if (isH264iFrame(ioState->fBuffer->dataStart()) &&
        time(nullptr) - ioState->fOurSink.fCurFileStart > ioState->fOurSink.fFileDuration) {
        ioState->fOurSink.rotateOutputFile();
...
void QuickTimeFileSink::onSourceClosure1() {
    ...
    while ((subsession = iter.next()) != NULL) {
        ...
        if (ioState->fOurSourceIsActive)
            return; // this source hasn't closed
    }
...
void SubsessionIOState::reset() {
    ...
    delete fHeadSyncFrame;
    fHeadSyncFrame = nullptr;
    fTailSyncFrame = nullptr;
...
SubsessionIOState::SubsessionIOState(QuickTimeFileSink& sink,
                                     MediaSubsession& subsession)
          : fHintTrackForUs(NULL),
            fTrackHintedByUs(NULL),
            fOurSink(sink),
            fOurSubsession(subsession),
            fLastPacketRTPSeqNum(0),
            fHaveBeenSynced(False),
            fQTTotNumSamples(0),
            fHeadChunk(NULL),
            fTailChunk(NULL),
            fNumChunks(0),
...
void SubsessionIOState::useFrame(SubsessionBuffer& buffer) {
    ...
    if (!fOurSink.fSyncStreams ||
        ...
        if (avcHack)
            frameSizeToUse += 4; // H.264/AVC gets the frame size prefix
        ...
        if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
            ...
            if (duration < 0.0)
                duration = 0.0;
            unsigned frameDuration = (unsigned)((2*duration*fQTTimeScale+1)/2); // round
            unsigned frameSizeToUse = fPrevFrameState.frameSize;
            if (avcHack)
                frameSizeToUse += 4; // H.264/AVC gets the frame size prefix

            unsigned numSamples = useFrame1(frameSizeToUse, ppt, frameDuration, fPrevFrameState.destFileOffset);
...
void SubsessionIOState::useFrameForHinting(unsigned frameSize,
    ...
    if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
        ...
        unsigned hintSampleDuration = (unsigned)((2*duration*fQTTimeScale+1)/2); // round
        if (hackm4a) {
            ...
            if (fTrackHintedByUs->fQTTimeScale != fOurSubsession.rtpTimestampFrequency()) {
                unsigned const scalingFactor = fOurSubsession.rtpTimestampFrequency()/fTrackHintedByUs->fQTTimeScale ;
        ...
        if (haveSpecialHeaders) { // special case
            ...
            immediateDataBytesRemaining = fPrevFrameState.specialHeaderBytesLength;
        }
        unsigned hintSampleSize = fOurSink.addHalfWord(numPTEntries);// Entry count
        ...
        for (unsigned i = 0; i < numPTEntries; ++i) {
            ...
            if (haveSpecialHeaders) { // special case
                ...
                if (hack263) {
                    Boolean PbitSet = immediateDataLen >= 1 && (immediateDataPtr[0]&0x4) != 0;
                    if (PbitSet) {
    ...
    if (hack263) {
        ...
        fPrevFrameState.packetSizes[0] = fPrevFrameState.specialHeaderBytesLength + frameSize;
    }
}
...
::ChunkDescriptor(int64_t offsetInFile, unsigned size,
                  ...
                  struct timeval presentationTime)
      : fNextChunk(NULL),
        fOffsetInFile(offsetInFile),
        fNumFrames(size/frameSize),
        fFrameSize(frameSize),
        fFrameDuration(frameDuration),
        fPresentationTime(presentationTime) {
}
...
addAtom(elst);
    ...
    if (totalDurationOfEdits > fCurrentIOState->fQTDurationM) {
        ...
        double scaleFactor = fCurrentIOState->fQTTimeScale/(double)movieTimeScale();
        fCurrentIOState->fQTDurationT = (unsigned)(totalDurationOfEdits*scaleFactor);
    }
addAtomEnd;
